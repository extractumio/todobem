...
| D26 | Long tool runs | exposure | root `wait_worker` segments the agent's own command produced: `sleep`, `poll-loop`, `tail -f` (never the harness's `agent` / `wait`, a `ci` watch or a `hook`) | their time, by kind; the per-run average is the interval the agent chose; what was waited for is not inferred |
