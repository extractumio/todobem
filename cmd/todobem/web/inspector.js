...
function inspectWait(laneId, ta, tb) {
  ...
  const dlg = $('#inspector');
  dlg.innerHTML = `<div class="dialog-head"><span class="eyebrow">Interval</span><button class="btn icon-only ghost" data-action="close-inspector" aria-label="Close" autofocus>${icon('close')}</button></div><div class="dialog-body"><span class="chip"><i class="color-square" style="background:${PHASES[isUser ? 'wait_user' : 'idle'].color}"></i>${label}</span><h2 id="inspectorTitle">${fmt(tb - ta, true)} · ${esc(l.path)}</h2><p class="desc">${esc(stampS(ta))} → ${esc(stampS(tb))}. No agent activity in this span — the model had produced its answer and was waiting.</p><h3>${lastHead}</h3><pre class="event-log">${esc(finalText || '(no final message recorded for the preceding turn)')}</pre><div class="dialog-actions"><button class="btn" data-action="jump" data-t="${Math.round(finalT)}">${icon('expand', true)}Jump to that moment</button></div><h3>${isUser ? 'Next user message' : 'Next instruction from the parent'}</h3><pre class="event-log">${nextUser ? esc(nextUser.text) : '(the session ended while waiting)'}</pre></div>`;
  if (!dlg.open) dlg.showModal();
}
...
async function inspectLane(id) {
  ...
  const name = l.depth === 0 ? 'root' : l.path.split('/').pop();
  const facts = [
    ['Nickname', l.nickname || '—'],
  ...
  const promptNote = !prompt ? 'No starting message is recorded in this rollout.'
    ...
    : '';
  const dlg = $('#inspector');
  dlg.innerHTML = `<div class="dialog-head"><span class="eyebrow">${l.depth ? 'Sub-agent' : 'Root agent'}</span><button class="btn icon-only ghost" data-action="close-inspector" aria-label="Close" autofocus>${icon('close')}</button></div>`
    ...
    + `<h3>${promptHead}${prompt ? ' · ' + stampS(prompt.marker.t) : ''}</h3><p class="desc" id="lanePromptNote">${esc(promptNote)}</p><pre class="event-log" id="lanePromptText">${esc(prompt ? prompt.marker.text : '(not recorded)')}</pre>`
    + `<h3>Final answer</h3><pre class="event-log">${esc(final || '(no final answer recorded)')}</pre>`
