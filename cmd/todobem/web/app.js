...
// PHASES is the activity palette (the fill of a lane). Hue = stage, tint = activity: an activity
// that serves a stage by default (classify.PhaseLifecycle and the pairing the legend shows —
// Development → Implementation, Testing → Verification, Release & deploy → Deployment,
// Infrastructure → Maintenance / operations) is drawn in a lighter tint of that stage's hue
// (LIFECYCLES below: L + .08 and chroma × .9 in OKLab, ≥ 5.6:1 on the surface), so the two
// legends read as one family per concept while the fill is never the stage's own colour. The
// activities no stage owns (model output, build, waits, compaction, gaps, unknown) keep their
// own hues, none of them a stage's.
const PHASES = {
  llm: { name: 'LLM', short: 'LLM', color: '#348989', kind: 'model' },
  code: { name: 'Development', short: 'Dev', color: '#68a9e2', kind: 'work' }, // the JSON key stays `code`: overlays, caches and the API reference it
  build: { name: 'Build', short: 'Build', color: '#a889fc', kind: 'work' },
  test: { name: 'Testing', short: 'Test', color: '#f0d76c', kind: 'work' },
  release: { name: 'Release & deploy', short: 'Release', color: '#6dbd7c', kind: 'work' },
  infra: { name: 'Infrastructure', short: 'Infra', color: '#dbaf8c', kind: 'work' },
...
const SUBGROUPS = {
  ...
  build: { compile: 'Compiling & bundling', deps: 'Installing dependencies' },
...
// Hue = stage: each of the eight has its own hue, and the stage is the pure colour — the band
// above a lane, the strip under the overview, the ring, a rail in the breakdown and the legend
// pipeline. An activity that serves a stage by default is the lighter tint of that hue (PHASES
// above), never the same hex: the two partitions are one family per concept and still never
// one mark. Every stage hue carries dark ink where text sits on it (≥ 4.2:1 on all eight; white
// is under 2:1 on the pale ones). Decision of 2026-09-19 (§13): a one-hue tone ramp for the
// stages was tried and rejected the same day — the stage is the concept and keeps the hue, the
// activity is the derived mark.
const LIFECYCLES = {
  ...
  test: { name: 'Verification', short: 'Test', color: '#d9bc3d', work: true },
...
// toolFailure: a failure the timeline draws as the red × in the lane — not a background op, and
// not an LLM failure (invalid tool call, broken exec script), which is the orange × marker in
// the row above; the legend row and the glyph share this one predicate.
const toolFailure = o => failure(o) && !o.background && o.kind !== 'exec-script-error' && baseKind(o.kind) !== 'llm-invalid-args';
...
// loadFailure explains a session that did not load: an agent that did not answer (502) is a
// transient, with the retry and the Servers page as the way out; anything else is the message.
function loadFailure(e) {
  const gateway = /^502 /.test(e.message || '');
  const head = gateway ? 'The agent did not answer' : 'Could not load session';
  const help = gateway ? `<p>The session is on a paired server that did not answer just now. Its last-known state stays in the list; <a href="#" data-action="retry-session">try again</a>, or see the Servers section of Settings for when the hub last reached it.</p>` : '';
  return `<div class="empty"><h3>${esc(head)}</h3><p class="mono">${esc((e.message || '').replace(/^\d{3} /, ''))}</p>${help}</div>`;
}
...
function windowStats(a, b) {
  ...
  const bySub = {}, allBySub = {}; // exclusive time per "phase:subgroup" (the winning slice's subgroup)
  const shared = {}, allShared = {}; // the part of each phase that is an equal share of a compound command (an estimate)
  const sharedLc = {}, allSharedLc = {}; // the same part per lifecycle key: a stage row names it like an activity row does
  const addSub = (into, sg, ov) => { if (sg.sub) { const k = `${sg.p}:${sg.sub}`; into[k] = (into[k] || 0) + ov; } };
  const addShared = (into, intoLc, sg, ov) => { if (sg.shared) { into[sg.p] = (into[sg.p] || 0) + ov; const lc = lifecycleOf(sg); intoLc[lc] = (intoLc[lc] || 0) + ov; } };
  for (const sg of r.segments) {
    ...
    by[sg.p] += ov;
    addSub(bySub, sg, ov);
    addShared(shared, sharedLc, sg, ov);
  ...
  for (const l of m.lanes) {
    for (const sg of l.segments) {
      ...
      addShared(allShared, allSharedLc, sg, ov);
...
// replaceHash rewrites the address of the page shown without a history entry: the filters of
// a report change what the address names, not where the reader is.
function replaceHash(h) { try { if (location.hash !== h) history.replaceState(null, '', h); } catch (e) { } }
function go(page, id) {
  ...
  if (page === 'insights') {
    setHash(insightsHash(insightsState().params));
    render();
    loadInsights();
    window.scrollTo(0, 0);
    return;
  ...
  if (page === 'session' && id && id !== state.id) {
    ...
    loadSession(id).then(async applied => {
      ...
      console.error(e); $('#main').innerHTML = loadFailure(e);
    });
    return;
  }
  ...
  if (page === 'sessions') {
    setHash(fleetHash());
    loadSessions().then(applied => { if (applied && navigation === navigationRequest) { render(); replaceHash(fleetHash()); } }).catch(e => {
...
const FLEET_TEXT = {
  ...
  clear: 'Clear the filters: last 30 days, every project, host and source, no search, recently updated first',
  sort: { updated: 'Recently updated', size: 'Largest logs', agents: 'Most sub-agents', started: 'Started first' },
  columns: ['Session', 'Host', 'Started', 'Updated', 'Elapsed', 'Sub-agents', 'Log size'],
  ...
  badLink: 'The link carried a filter this page could not read; showing the list as it stands.',
};
// The sessions-list bar — period, project, host, sources, search, sort — is remembered in one
// cookie and restored on the next visit; a remembered project or host that the list no longer
// holds falls back to All (reconcileFleetFilter, after every list load). Clear puts the
// defaults back.
const FLEET_DEFAULTS = () => ({ kind: '30d', from: '', to: '', cwd: '', host: '', sources: {}, search: '', sort: 'updated' });
function fleetPrefs() {
  const f = state.fleetFilter;
  return { kind: f.kind, from: f.from, to: f.to, cwd: f.cwd, host: f.host, sources: f.sources || {}, search: state.search, sort: state.fleetSort };
}
function persistFleetFilter() {
  prefSet('todobem_fleet', JSON.stringify(fleetPrefs()));
  if (state.page === 'sessions') replaceHash(fleetHash()); // the address names the list as it is now
}
// fleetHash is the list's address: #sessions, or #sessions?f=<word> once the filter, the search
// or the sort is off the default (filterToLink over fleetPrefs; the default sort is left out).
function fleetHash() {
  if (fleetFilterIsDefault()) return '#sessions';
  const p = fleetPrefs();
  if (p.sort === FLEET_DEFAULTS().sort) delete p.sort;
  return '#sessions?f=' + filterToLink(p);
}
// applyFleetLink reads a list address into the bar — the word's fields over the defaults, since
// an address describes the whole view — and says whether it carried one; an unreadable word is
// said so and applies nothing.
function applyFleetLink(hash) {
  const word = /(?:^|&)f=([^&]*)/.exec(hash.startsWith('#sessions?') ? hash.slice('#sessions?'.length) : '');
  if (!word) return false;
  const f = filterFromLink(word[1]);
  if (!f) {
    toast(FLEET_TEXT.badLink);
    return false;
  }
  const d = FLEET_DEFAULTS();
  state.fleetFilter = { kind: f.kind, from: f.from, to: f.to, cwd: f.cwd, host: f.host, sources: f.sources };
  state.search = f.search || d.search;
  state.fleetSort = FLEET_TEXT.sort[f.sort] ? f.sort : d.sort;
  persistFleetFilter();
  return true;
}
function restoreFleetFilter() {
  try {
    const v = JSON.parse(prefGet('todobem_fleet') || 'null');
    if (!v || !FILTER_KINDS.includes(v.kind)) return;
    const f = state.fleetFilter;
    f.kind = v.kind;
    f.from = v.from || '';
    f.to = v.to || '';
    f.cwd = typeof v.cwd === 'string' ? v.cwd : '';
    f.host = typeof v.host === 'string' ? v.host : '';
    f.sources = v.sources && typeof v.sources === 'object' ? v.sources : {};
    state.search = typeof v.search === 'string' ? v.search : '';
    state.fleetSort = FLEET_TEXT.sort[v.sort] ? v.sort : 'updated';
  } catch (e) { }
}
// reconcileFleetFilter drops a remembered project or host the list does not hold any more.
function reconcileFleetFilter() {
  const f = state.fleetFilter;
  if (f.cwd && !state.sessions.some(s => s.cwd === f.cwd)) f.cwd = '';
  if (f.host && f.host !== LOCAL_HOST && !state.sessions.some(s => hostOf(s) === f.host)) f.host = '';
  if (f.host === LOCAL_HOST && !state.sessions.some(s => hostOf(s) === '')) f.host = '';
}
function resetFleetFilter() {
  const d = FLEET_DEFAULTS();
  state.fleetFilter = { kind: d.kind, from: d.from, to: d.to, cwd: d.cwd, host: d.host, sources: d.sources };
  state.search = d.search;
  state.fleetSort = d.sort;
  persistFleetFilter();
}
// fleetFilterIsDefault: nothing to clear
function fleetFilterIsDefault() {
  const d = FLEET_DEFAULTS(), p = fleetPrefs();
  return p.kind === d.kind && !p.cwd && !p.host && !p.search && p.sort === d.sort && !SOURCE_ORDER.some(k => p.sources[k] === false);
}
// fleetPage: the heading, one bar with the shared period + project + host + source filter
// (filter.js) next to the search, the sort and Clear, the summary tiles of the sessions in
// view, then the rows.
function fleetPage() {
  const T = FLEET_TEXT;
  ...
  const clear = `<button class="btn icon-only ghost" type="button" data-action="fleet-clear" title="${esc(T.clear)}" aria-label="${esc(T.clear)}" ${fleetFilterIsDefault() ? 'disabled' : ''}>${icon('trash')}</button>`;
...
<section class="report-bar" aria-label="Sessions period, project, host, source, search and sort">${filter}<div class="bar-actions bar-fill">${search}<span class="bar-tail">${sort}${clear}</span></div></section>
...
const fleetRank = (s, now) => isActive(s, now) ? 0 : awaiting(s, now) ? 1 : 2;
...
// line (project, branch, model, harness version). withHost puts the host chip before the title
// (the tile, which has no Host column).
function sessionLink(s, withHost = false) {
...
function renderFleetRows() {
  ...
  const matches = s => filterKeeps(f, s, now) && `${s.title} ${s.cwd} ${s.branch || ''} ${s.id} ${sourceName(s)} ${hostOf(s)}`.toLowerCase().includes(q);
  ...
  // the status chips sit on their own row under the timestamp, so every row places them at
  // the same edge whatever the column width — inline after the stamp they wrapped, or not,
  // per row
  const status = s => {
    const chips = activeChip(s) + askChip(s, now);
    return chips ? `<div class="status">${chips}</div>` : '';
  };
  const tableRow = s => `<tr><td>${sessionLink(s)}</td><td>${hostMark(s)}</td><td class="mono">${stamp(s.started)}</td><td class="mono">${stamp(s.updated)}${status(s)}</td><td>${stat(s)}</td><td class="mono">${s.agents}</td><td class="mono">${(s.bytes / 1e6).toFixed(1)} MB</td></tr>`;
  const tile = s => `<article class="session-tile">${sessionLink(s, true)}${askChip(s, now)}<div class="session-tile-metrics"><div><strong class="mono">${stamp(s.updated)}</strong><small>Updated</small></div><div><strong class="mono">${s.agents}</strong><small>Sub-agents</small></div><div><strong class="mono">${(s.bytes / 1e6).toFixed(0)} MB</strong><small>Log</small></div></div></article>`;
...
// remoteHosts: the paired agents the list holds sessions of.
function remoteHosts() {
  return hostsPresent(state.sessions || []).filter(Boolean);
}
// footer names where the data comes from: this machine's files, and the paired agents when the
// list holds their sessions (the hub dials them and nothing else).
function footer() {
...
function sidebarOrigin() {
  ...
  if (!note) return;
  const n = remoteHosts().length;
  const text = `Codex & Claude Code logs · ${n ? `local + ${plural(n, 'paired server')}` : 'local only'}`;
  if (note.firstChild && note.firstChild.nodeType === 3) note.firstChild.textContent = text;
  else note.textContent = text;
}
// refreshSessionList reloads the list after something changed its contents (the folders, the
// paired servers) and redraws the Sessions page if it is the one shown.
...
function sessionPage() {
  ...
  // the legend lists what this session drew, not the palette: an activity or a stage no lane
  // spent time in is not a colour on the page, so it is not an entry. Every lane counts — the
  // fill and the band of a sub-agent lane too; a live update rebuilds the page, so an entry
  // appears with the first segment that needs it.
  const present = key => new Set(m.lanes.flatMap(l => Object.keys(l[key] || {}).filter(k => l[key][k] > 0)));
  const phasesPresent = present('by_phase');
  const stagesPresent = present('by_lifecycle');
  const none = text => `<span class="row note">${text}</span>`;
  // the phase legend in order of kind: what the agent did, then waiting, overhead and gaps
  const legend = kinds => PHASE_ORDER.filter(k => k !== 'idle' && kinds.includes(PHASES[k].kind) && phasesPresent.has(k)).map(k => `<span class="row">${swatch(k)}${PHASES[k].name}</span>`).join('');
  const activityLegend = legend(['model', 'work']) + legend(['wait', 'overhead', 'unknown']) || none('No activity recorded');
  // the stage legend is the pipeline itself: the stages this session served in lifecycle order,
  // each on its own hue — drawn like the strip under the overview, not as a second grid of
  // swatches beside the activity one
  // the marks legend under the chart lists only the glyphs the timeline draws for this session:
  // every lane's markers, turn ends, background bars and failure crosses decide the rows, in the
  // order they were always listed; a session with none of them has no marks legend at all
  const anyMarker = kind => m.lanes.some(l => l.markers.some(k => k.kind === kind));
  const anyTurn = status => m.lanes.some(l => l.turns.some(t => t.status === status));
  const anyOp = test => m.lanes.some(l => l.ops.some(test));
  const cross = color => `<svg width="12" height="12"><path d="M2 2l8 8m0-8l-8 8" stroke="${color}" stroke-width="2.4" stroke-linecap="round"/></svg>`;
  const markRows = [
    [anyMarker('user_message'), `<svg width="12" height="12">${userGlyph(6, 0, MARKS.user_message.color, .85)}</svg>`, 'User message'],
    [anyMarker('final_answer'), `<svg width="12" height="12"><circle cx="6" cy="6" r="5" fill="${MARKS.final_answer.color}"/></svg>`, 'Final answer'],
    [anyOp(o => o.background), `<svg width="14" height="12"><rect x="0" y="5" width="14" height="3" rx="1.5" fill="#c9a15c"/></svg>`, 'Background process'],
    [anyTurn('completed'), `<svg width="12" height="12"><circle cx="6" cy="6" r="5.5" fill="#4aa65f"/><path d="M3 6l2 2 4-4" stroke="#0f1b23" stroke-width="1.6" fill="none"/></svg>`, 'Turn completed'],
    [anyTurn('aborted') || anyMarker('interrupted'), `<svg width="12" height="12"><circle cx="6" cy="6" r="5.5" fill="#d76368"/><path d="M3.5 3.5l5 5m0-5l-5 5" stroke="#0f1b23" stroke-width="1.6"/></svg>`, 'Interrupted'],
    [anyTurn('orphaned'), `<svg width="12" height="12"><circle cx="6" cy="6" r="5" fill="#22313d" stroke="#7d8fa1" stroke-width="1.5"/><path d="M2.5 9.5l7-7" stroke="#7d8fa1" stroke-width="1.5"/></svg>`, 'Never closed'],
    [LIFECYCLE_ORDER.some(k => LIFECYCLES[k].work && stagesPresent.has(k)), `<svg width="14" height="12"><rect x="0" y="1" width="14" height="5" rx="1" fill="${LIFECYCLES.implement.color}" opacity=".35"/><rect x="0" y="5" width="14" height="2" fill="${LIFECYCLES.implement.color}"/><rect x="0" y="8" width="14" height="4" rx="1" fill="#22313d"/></svg>`, 'Stage band (the SDLC stage the time served, above the raw fill; colours above)'],
    [anyOp(toolFailure), cross('#e5484d'), 'Tool failure (non-zero exit)'],
    [anyMarker('llm_error'), cross(MARKS.llm_error.color), 'LLM failure: invalid tool call / broken exec script / API error'],
  ];
  const markRowsShown = markRows.filter(([shown]) => shown).map(([, glyph, label]) => `<span class="row">${glyph}${label}</span>`).join('');
  const marksLegend = markRowsShown ? `<div class="legend legend-marks" id="legendMarks">${markRowsShown}</div>` : '';
  const pipelineLegend = LIFECYCLE_ORDER.filter(k => LIFECYCLES[k].work && stagesPresent.has(k)).map(k => `<span class="lc-step" style="background:${LIFECYCLES[k].color}" data-lc="${k}">${LIFECYCLES[k].name}</span>`).join('') || none('No stage recorded — no tool call served one');
  return `<section class="page-heading"><div><div class="eyebrow"><span class="source-tag source-${sourceOf(m)}">${esc(sourceName(m))}</span> Session ${esc(m.id.slice(0, 8))} · ${esc(m.model || '?')} · ${esc(sourceName(m))} ${esc(m.cli || '?')}</div><h1>${esc(m.title)}</h1><p class="subtitle">${hostMark(m)}${icon('repo', true)}${esc(m.cwd)}${m.branch ? `<span>·</span>${esc(m.branch)}` : ''}<span>·</span>${stamp(m.started)} — ${stamp(m.ended)} ${esc(TZ)}</p></div><div class="heading-actions">${status}<label class="chip">Refresh <select class="select compact" id="intervalSelect">${[[30, '30s'], [60, '1 min'], [120, '2 min'], [300, '5 min']].map(([v, l]) => `<option value="${v}" ${state.interval === v ? 'selected' : ''}>${l}</option>`).join('')}</select></label><button class="btn" data-action="refresh">${icon('refresh', true)}Refresh now</button></div></section>
...
<div class="chart-footer"><span class="chart-note" id="chartNote"></span><span class="chart-shortcuts"><kbd>+</kbd> <kbd>−</kbd> zoom · <kbd>←</kbd> <kbd>→</kbd> pan · <kbd>Home</kbd> fit · drag to pan</span></div>${marksLegend}</section>
...
function renderTimeline() {
  ...
  list.forEach((row, ri) => {
    ...
    if (row.kind === 'lane') {
      ...
      // an equal share of a compound command is an estimate: a light hatch over the fill says so
      for (const sg of segsVis) {
        if (!sg.shared) continue;
        const xa = x(Math.max(sg.s, state.a)), xb = x(Math.min(sg.e, state.b));
        if (xb - xa < 1.5) continue;
        html += `<rect class="est" x="${xa.toFixed(1)}" y="${top0}" width="${(xb - xa).toFixed(1)}" height="${fh}" fill="url(#estHatch)" pointer-events="none"/>`;
      }
      ...
      { let lastX = -99;
        const fy = y + rh - 4;
        for (const o of l.ops) {
          if (!toolFailure(o) || o.start < state.a || o.start > state.b) continue;
          const xx = x(o.start);
          const cy = fy - 4;
...
function renderLower() {
  ...
  // rows of a section add up to 100 % — the stage rows together with the waits, gaps and unknown
  // they leave to the Activity list; the two sub-agent rows are not a partition and show none
  const pct = (n, total) => (total ? n / total * 100 : 0).toFixed(1) + '%';
  const rows = (entries, total) => entries.filter(([, n, , , , count]) => n > 0 || count == null || count > 0).map(([k, n, def, sel, data, count, sub, cls]) => `<button class="breakdown-item${cls ? ' ' + cls : ''}${sel ? ' active' : ''}" ${data} title="${esc(def.name)}: ${fmt(n, true)}${sub ? ' · ' + esc(sub) : ''}${count == null ? '' : ` · ${plural(count, INTERVAL_PHASES.has(k) ? 'interval' : 'operation')} in the list · ${pct(n, total)} of this section`}"><span class="name">${swatchFor(def)}<span class="label">${def.name}</span>${sub ? `<small class="row-sub">${esc(sub)}</small>` : ''}</span><span class="bar-track"><span class="bar-fill" style="width:${total ? clamp(n / total * 100, 0, 100) : 0}%;background:${def.color}"></span></span><span class="count num">${count == null ? '' : count}</span><span class="share num">${count == null ? '' : pct(n, total)}</span><span class="time num">${def.est ? '≈ ' : ''}${fmt(n)}</span></button>`).join('');
  ...
  // Non-stage time keeps its activity name in the stage partition, so "Outside stages" lists only
  // what the Activity list below does not say (the guide's Lifecycle section, ARCHITECTURE §9.3):
  // the model output no stage could take; a wait or compaction by its remainder when a turn's
  // stage took the rest of it (the row says how much more the stages hold); Unknown with its
  // model / tools split when it differs (the model output around an unknown command is unknown
  // too). The heading totals what the stages took from the waits, gaps and unknown, so a wait a
  // stage took whole is not silently counted twice.
  const passesThrough = k => k !== 'llm' && (lcSrc[k] || 0) === (src[k] || 0);
  const taken = k => Math.max(0, (src[k] || 0) - (lcSrc[k] || 0));
  const absorbed = lcRows.filter(k => !LIFECYCLES[k].work && k !== 'llm').reduce((n, k) => n + taken(k), 0);
  // a stage row names the compound-command share inside it the way an activity row does
  // (product rule 3: the estimate is named wherever it is shown): ≈ on the number, the amount
  // in the label
  const sharedLcSrc = state.allLanes ? t.allSharedLc : t.sharedLc;
  const lcEntries = lcRows.filter(k => LIFECYCLES[k].work || !passesThrough(k)).map(k => {
    const n = lcSrc[k] || 0;
    const sp = lcSplitSrc[k] || { llm: 0, tools: 0 };
    const work = LIFECYCLES[k].work;
    const split = (work || k === 'unknown') && n;
    const est = sharedLcSrc[k] || 0;
    const notes = [split ? `model ${fmt(sp.llm)} · tools ${fmt(sp.tools)}` : !work && k !== 'llm' ? `${fmt(taken(k))} more inside the stages above` : '', est ? `≈ ${fmt(est)} shared from compound commands` : ''];
    const sel = INTERVAL_PHASES.has(k) ? state.phase === k && state.lifecycle === 'all' : state.lifecycle === k;
    return [k, n, est ? { ...LIFECYCLES[k], est: true } : LIFECYCLES[k], sel, `data-action="filter-lifecycle" data-lc="${k}"`, countOfLc(k), notes.filter(Boolean).join(' · ')];
  });
  ...
  const lcNote = `same time, by SDLC stage · waits, gaps and unknown: see Activity${absorbed ? ` · the stages hold ${fmt(absorbed)} of them` : ''}`;
  const lcSection = `<div class="mini-heading"><h3>Lifecycle stage</h3><span class="mini-note">${lcNote}</span></div>` + rows(order(stageEntries), denom)
  ...
  const sharedSrc = state.allLanes ? t.allShared : t.shared;
  const phaseNote = k => {
    const est = sharedSrc[k] || 0;
    if (est) return `≈ ${fmt(est)} shared from compound commands`;
    return PHASES[k].kind === 'work' ? 'tool calls only' : '';
  };
  const phaseDef = k => sharedSrc[k] ? { ...PHASES[k], est: true } : PHASES[k];
  const phaseEntries = order(phaseRows.map(k => [k, src[k], phaseDef(k), state.phase === k, `data-action="filter" data-phase="${k}"`, countOf(k, 'all'), phaseNote(k)])).flatMap(e => [e, ...subEntries(e[0])]);
...
// shareOf is the slice of a compound command booked to a phase (Operation.shares), if any.
const shareOf = (o, phase) => (o.shares || []).find(sh => sh.phase === phase);
// sharesText names a compound command's split for a label: "build ½ · test ½".
function sharesText(o) {
  const n = (o.shares || []).length;
  if (!n) return '';
  return o.shares.map(sh => `${PHASES[sh.phase]?.name || sh.phase} ${sh.literal ? fmt(sh.ms) : fmt(sh.ms) + ' (1/' + (n - o.shares.filter(x => x.literal).length) + ')'}`).join(' · ');
}
...
function opMatches(o, phase, role, lifecycle = state.lifecycle, sub = state.sub) {
  ...
  if (phase !== 'all' && o.phase !== phase && !shareOf(o, phase)) return false;
  if (sub !== 'all' && (phase !== 'all' && o.phase !== phase ? !(o.shares || []).some(sh => sh.phase === phase && sh.sub === sub) : o.sub !== sub)) return false;
...
function renderConversation() {
  ...
  // every message is rendered by harness.js (the tags a message carries as structure, the text
  // as Markdown); a harness marker's first line is its ref. The adapters clip a harness message,
  // so a clipped one offers the marker dialog, which completes it from the source event
  const body = k => prose(k.text, k.kind === 'system_message' ? k.ref : '');
  const clipped = k => k.kind === 'system_message' && k.text.endsWith('…');
  const item = k => {
    const key = `${k.t}:${k.kind}`;
    const open = state.convOpen.has(key);
    const inWin = k.t >= state.a && k.t <= state.b;
    const long = k.text.length > 380;
    const full = clipped(k) ? `<button class="text-btn" data-action="marker" data-mk="${esc(`${r.id}:${k.t}:${k.kind}`)}" style="margin-top:4px;margin-left:10px" title="The recorded text is clipped here; the marker dialog reads all of it">Full text</button>` : '';
    return `<div class="conv-item ${cls(k)}" style="${inWin ? '' : 'opacity:.55'}"><button class="conv-time" data-action="jump" data-t="${k.t}" title="Jump to this moment"><b>${label(k)}</b>${stamp(k.t)}</button><div><div class="conv-text ${proseView()} ${open ? 'open' : ''}">${body(k)}</div><button class="text-btn" data-action="conv-toggle" data-key="${esc(key)}" style="margin-top:4px" ${long || open ? '' : 'hidden'}>${open ? 'Show less' : 'Show all'}</button>${full}</div></div>`;
  };
...
// lifecycleGuideHTML is the guide's Lifecycle section: the second partition explained, one
// sentence per line so a change to one reads in a diff
function lifecycleGuideHTML() {
  const sentences = [
    `Every millisecond of a lane also belongs to exactly one <em>lifecycle stage</em>: the stage of the software lifecycle the time served. `,
    `It is a second partition next to the activity phases (same total), never a replacement: a code review still runs tests, and those minutes stay Testing in the activity list while they are Code review here (the stage Verification also holds lint and type checks). `,
    `Each stage row shows its model / tools split because the two partitions attribute model output differently. `,
    `Every assignment comes from a literal, harness-level signal — a collaboration mode, a skill the harness actually injected, an agent's spawn role, an edited path, a command kind — never from words in a message and never from a duration. `,
    `Requirements and design have no built-in detector: nothing in a Codex rollout or a Claude Code log marks them; add skill names, agent roles or document paths in your overlay. `,
    `Model output takes the stage of the nearest tool call in its turn — the next one (the call it prepared), else the previous one (the answer that reported on it); a compaction or a telemetry gap in between is skipped over, while a wait for a worker or the user ends that attribution. `,
    `Only a turn that made no tool call at all keeps its model output as <em>Model output, no tool call</em>: nothing in it says which stage it served, and only a harness signal (plan mode, a skill, a plan written with <code>update_plan</code> or <code>ExitPlanMode</code>) could. `,
    `The stage list holds the SDLC stages only; waiting, compaction, telemetry gaps and unknown commands are not stages — they happened inside or between them and keep their activity name in the stage partition, so the Activity list holds them, with the same denominator: the stages, that model output and those rows add up to the whole. `,
    `A turn that has a stage — a skill run, plan mode, an agent's role, its parent's stage — reads as that stage for its whole wall clock, waits included, and an overlay may pin a wait or an unknown command to a stage too; the stage heading then says how much of the waits, gaps and unknown the stages hold, so nothing is counted twice. `,
    `<em>Outside stages</em> lists only what the Activity list does not say: that model output; a wait or compaction by its remainder outside every stage, saying how much more the stages hold; and Unknown with its model / tools split when it differs from the Activity row, because the model output around an unknown command is unknown too. `,
    `A turn is read as a group before any of its calls is decided: every code, build, test and infrastructure call between the turn's first and last change (an edit, a patch, a written file) is implementation — a test run between two edits is the loop, not QA — while a test after the last change is the verification pass, and a call before the turn's first plan that changed nothing is planning. `,
    `A skill pins its stage from the moment it was invoked to the end of the turn (Codex injects it at the turn start, so the whole turn; Claude Code's Skill tool call may come mid-turn). `,
    `A sub-agent's turn is a tool call of the parent turn it ran inside, so it takes that turn's stage when it has no signal of its own; the inspector names the origin.`,
  ];
  return `<section class="guide-section"><h3>Lifecycle stages (SDLC)</h3><p>${sentences.join('')}</p><div style="overflow:auto;max-height:340px"><table class="rules-table" id="lifecycleTable"></table></div></section>`;
}

async function guide() {
  const dlg = $('#guide');
  dlg.innerHTML = `<div class="dialog-head"><h2 id="guideTitle">Reading the timeline</h2><button class="btn icon-only ghost" data-action="close-guide" aria-label="Close" autofocus>${icon('close')}</button></div><div class="dialog-body"><section class="guide-section"><h3>One row per agent</h3><p>The ${MAIN_THREAD} (the ${MOTHER_AGENT}'s) is the first lane; every sub-agent thread is its own lane, indented by depth, with a ▶ at spawn, ticks at each message from the parent, and a bar at each completed turn. A dashed line connects spawn time to the parent. Only lanes alive inside the visible window are drawn. Click a lane to expand it into phase rows (and, at fine zoom, individual operations); the ⓘ at the right of its name opens the agent card: nickname, model, the prompt it was spawned with and its final answer.</p></section><section class="guide-section"><h3>Stage band above, raw fill below</h3><p>A lane's fill is the real exclusive time split: teal is LLM time (the model generating — reasoning, answers and the code of every patch), colours are tool phases, grey is waiting for the user, hatched is missing telemetry. Development is every tool call around the code short of building, testing and shipping: reading and searching sources, edits, local git, formatting, lookups and probes. The model's time writing a patch is LLM, not Development: the activity split never moves model output into a tool phase. Above the fill, the <em>stage band</em> names the SDLC stage the same time served (the second partition, below): Implementation, Verification, Code review … in the stage colours — each stage its own hue; an activity that serves a stage by default (Development → Implementation, Testing → Verification, Release → Deployment, Infrastructure → Maintenance) is drawn in a lighter tint of that hue, so the two are one family and never one mark — with the model / tools split in its tooltip. The band answers <em>why</em>, the fill answers <em>what</em>; waiting, compaction and telemetry gaps are not stages and leave the band empty. Turn ends carry ✓ (completed), ✕ (interrupted) or ⊘ (never closed); a pulsing dot means the turn is still open. Dashed pink verticals are turn starts.</p></section><section class="guide-section"><h3>What is inferred and what is not</h3><p>Phases come from a rule table over the command text (below). Turn boundaries give waiting-for-user time; a turn that never closed becomes "No telemetry". Retry groups only join operations with an identical normalized command. A failure is a test, build, release, infra step, edit or script that exited non-zero; a read, search, listing or probe that exited non-zero — or a CI status wait such as <code>gh pr checks</code> whose exit says the checks are pending or failing — is a <em>query miss</em> (the exit was its answer) — kept verbatim with its exit code, not counted. Token totals sum each thread's per-call usage, so a resumed thread whose counter restarted still adds up. Nothing is derived from how long something took, and no "goal reached" score exists — read the conversation panel.</p></section><section class="guide-section"><h3>Parallel time</h3><p>Session totals are the ${MAIN_THREAD}'s exclusive wall clock. Sub-agent time is summed separately and its union shown as "wall".</p></section><section class="guide-section"><h3>Code review</h3><p>A bookmark glyph marks where a review or cleanup skill (for example <code>code-review-cc</code> or <code>simplify</code>) was <em>actually invoked</em> — detected from the harness's skill-instruction injection, not from the words in a prompt. The stage band above the fill reads Code review for each turn a review skill ran in — and for the sub-agent turns spawned inside it, which work for that review — and the whole-session card totals the ${MAIN_THREAD}'s wall clock; it is a stage, not an activity, so the fill still shows the real LLM/development/wait split above it. Turns that reuse the skill later without re-invoking it are not counted. Detection has two sources: a built-in set of tools and skills, and an optional user overlay (project commands and review-skill names); both are listed below.</p></section><section class="guide-section"><h3>Keyboard</h3><p><kbd>+</kbd>/<kbd>−</kbd> zoom, <kbd>←</kbd>/<kbd>→</kbd> pan, <kbd>Home</kbd> fit, <kbd>Esc</kbd> close. Drag the plot to pan; drag on the overview to select.</p></section>${lifecycleGuideHTML()}<section class="guide-section"><h3>Classification rules</h3><p id="rulesNote">loading…</p><div style="overflow:auto;max-height:340px"><table class="rules-table" id="rulesTable"></table></div></section><section class="guide-section"><h3>Compound commands</h3><p>One tool call can chain work of several kinds: <code>go build ./... &amp;&amp; go test ./...</code>, <code>npm run build &amp;&amp; npm test</code>, a poll loop followed by a remote test run. The log records one start and one end for the whole call, nothing per part. Rather than booking the whole call to its strongest phase and leaving the others blind, todobem <em>shares</em> its wall clock: a <code>sleep N</code> part takes its literal seconds, and the rest is divided <em>equally</em> among the distinct categories the call chained — build, test, release, infrastructure, waiting (a poll loop) and unknown commands; reads, edits, git and pipe filters are glue and take nothing. The slices sit back to back in the order the parts ran, in the activity and the stage partitions alike, so the totals still add up; the raw op-sum follows the same split. This is an explicit estimate, never a measurement: the breakdown says how much of a phase came from such shares, the operation keeps its dominant phase, kind and retry identity, and the inspector lists the split. A call that chains work of one category only is not split.</p></section><section class="guide-section"><h3>Sub-rows of Development, Build, Waiting for workers and Unknown</h3><p>Four phases lump different work, so the breakdown lists them with sub-rows: what the tool calls read, searched, edited, versioned, fetched or ran; whether a build compiled the project or installed its dependencies. A sub-row is a fixed grouping of the kinds below (a function of the phase and the kind, never of a duration); its time is the exclusive partition of those calls, its count the calls in the window. Select one to list exactly those operations.</p><div style="overflow:auto;max-height:340px"><table class="rules-table" id="subgroupTable"></table></div></section>${insightsGuideHTML()}</div>`;
  dlg.showModal();
  try {
...
document.addEventListener('click', e => {
  ...
  if (a === 'filter-apply') { // custom dates of the shared filter: the Insights report, or the session list
    ...
    else if (filterApply('fleet', state.fleetFilter)) { persistFleetFilter(); render(); }
    return;
  }
  if (a === 'fleet-clear') { resetFleetFilter(); render(); return; }
  if (a === 'retry-session') { if (e.preventDefault) e.preventDefault(); const id = requestedSessionID; state.id = null; go('session', id); return; }
  ...
  if (a === 'marker') return inspectMarker(el.dataset.mk);
...
document.addEventListener('input', e => {
  ...
  else if (e.target.dataset && e.target.dataset.server !== undefined) serversInput(e.target.dataset.server, e.target.value);
});
// overview brush
...
function route() {
  ...
  // the report's addresses: #insights, #insights?f=<filter>, #session/<id>/insights
  if (h.startsWith('#insights') || SESSION_INSIGHTS_HASH.test(h)) {
    const applied = applyInsightsLink(h);
    if (state.page !== 'insights') go('insights');
    else if (applied) loadInsights();
    else replaceHash(insightsHash(insightsState().params)); // the address names what is shown
    return;
  }
  if (h === '#settings') {
  ...
  } else if (h.startsWith('#sessions?')) {
    // the list's address with a filter word: applied over the defaults, then the page as usual
    const applied = applyFleetLink(h);
    if (state.page !== 'sessions') go('sessions');
    else if (applied) render();
    else replaceHash(fleetHash());
...
async function boot() {
  ...
  if (!applied) return;
  if (location.hash.startsWith('#session/') || location.hash.startsWith('#sessions?') || location.hash.startsWith('#insights') || location.hash === '#settings') route();
  else go('sessions');
}
(async () => {
  const tok = tokenFromHash();
