...
// reading files, searching, editing, git, code hosting, network, shell; compiling, installing
// dependencies, test, release, infrastructure; waits for sub-agents, polling, hooks; unknown
...
func detectToolCalls(f *Facts) Result {
	...
	for _, t := range f.ToolCalls {
		...
		r.Stats["shared_ms_"+key] += t.SharedMs
		...
		if t.Shared > 0 {
			// an equal share of a compound call (`go build && go test`): an estimate, listed apart
			note += fmt.Sprintf(", %d compound %s shared here (estimate)", t.Shared, plural(t.Shared, "call"))
		}
